...
export class WebSearchTool {
  ...
  constructor(config: Partial<WebSearchConfig> = {}) {
    ...
    logger.debug('WEB_SEARCH', 'Web search tool initialized', { 
  ...
  async search(query: string): Promise<ToolResult> {
    try {
      logger.info('WEB_SEARCH', 'Performing web search', { 
      ...
      logger.info('WEB_SEARCH', 'Search completed successfully', {
      ...
      logger.error('WEB_SEARCH', 'Search failed', { 
        query: query.substring(0, 50),
        error: errorMessage,
        provider: this.config.provider
      });
  ...
  private async searchDuckDuckGo(query: string): Promise<SearchResult[]> {
    try {
      ...
      logger.warn('WEB_SEARCH', 'DuckDuckGo API failed, providing search URL', { 
  ...
  updateConfig(newConfig: Partial<WebSearchConfig>): void {
    ...
    logger.info('WEB_SEARCH', 'Configuration updated', { 
      provider: this.config.provider,
      maxResults: this.config.maxResults 
    });
  }
