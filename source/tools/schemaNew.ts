...
export const listDirectorySchema = z.object({
  ...
  respect_git_ignore: z.boolean().optional().default(true).describe("Whether to respect .gitignore patterns when listing files")
});
...
export const globSchema = z.object({
  ...
  path: z.string().optional().describe("The absolute path to the directory to search within. If omitted, searches the tool's root directory"),
  case_sensitive: z.boolean().optional().default(false).describe("Whether the search should be case-sensitive"),
  respect_git_ignore: z.boolean().optional().default(true).describe("Whether to respect .gitignore patterns when finding files")
});
...
export const replaceSchema = z.object({
  ...
  old_string: z.string().describe("The exact literal text to replace. Must uniquely identify the instance to change. Should include context lines. If empty, creates a new file"),
  new_string: z.string().describe("The exact literal text to replace old_string with"),
  expected_replacements: z.number().optional().default(1).describe("The number of occurrences to replace")
});
