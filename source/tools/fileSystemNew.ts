  ...
  try {
    ...
    if (respect_git_ignore) {
      entries = entries.filter(entry => {
        ...
        const commonIgnored = ['.git', 'node_modules', '.DS_Store', '*.tmp', '*.log'];
...
export async function readFile(args: {
  ...
  try {
    ...
    if (imageExtensions.includes(extension)) {
      ...
      const mimeType = `image/${extension.slice(1) === 'jpg' ? 'jpeg' : extension.slice(1)}`;
      return {
        success: true,
    ...
    if (offset !== undefined && limit !== undefined) {
      ...
      truncationMessage = `[File content truncated: showing lines ${offset + 1}-${Math.min(offset + limit, lines.length)} of ${lines.length} total lines]\n`;
...
export async function glob(args: {
  ...
  try {
    ...
    const options: any = {
      ...
      ignore: respect_git_ignore ? ['node_modules/**', '.git/**', '*.log', '*.tmp'] : []
    };
    ...
    const output = `Found ${matches.length} file(s) matching "${pattern}" within ${searchPath}, sorted by modification time (newest first):\n${sortedFiles.join('\n')}`;
  ...
  try {
    ...
    try {
      ...
      if (!stdout.trim()) {
        return {
          success: true,
          output: `No matches found for pattern "${pattern}" in path "${searchPath}"${include ? ` (filter: "${include}")` : ''}`
        };
      }
      ...
      let output = `Found ${lines.length} matches for pattern "${pattern}" in path "${searchPath}"${include ? ` (filter: "${include}")` : ''}:\n---\n`;
...
export async function replace(args: {
  ...
  const { file_path: filePath, old_string: oldString, new_string: newString, expected_replacements = 1 } = args;
